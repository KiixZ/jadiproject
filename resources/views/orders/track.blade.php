                    ...
                    <div class="ml-4">
                        ...
                        <p class="mt-1 text-sm text-gray-500">
                            @if ($order->payment_status === 'paid')
                                {{ $order->transaction && $order->transaction->payment_date ? \Carbon\Carbon::parse($order->transaction->payment_date)->format('d M Y, H:i') : 'Sudah Dibayar' }}
                            @else
                                @if ($order->payment_method === 'midtrans')
                                    Menunggu pembayaran melalui Midtrans (otomatis update setelah pembayaran)
                                @elseif ($order->payment_method === 'transfer')
                                    Menunggu upload & verifikasi bukti transfer
